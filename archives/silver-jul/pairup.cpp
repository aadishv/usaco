...
int main() {
    ...
    while (a <= b) {
        if (a == b) {
            worst = max(worst, types[0].first * 2);
            break;
        }
        auto f = types[a], l = types[b];
        worst = max(worst, l.first + f.first);
        if (f.second <= l.second) {
            types[b].second -= f.second;
            a++;
        }
        if (l.second <= f.second) {
            types[a].second -= l.second;
            b--;
        }
        // for (auto& t: types) {
        //     cout << t.second << " of " << t.first << ", ";
        // }
        // cout << endl;
    }
    cout << worst << endl;
